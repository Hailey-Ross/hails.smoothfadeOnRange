...
float current_alpha = 1.0; // 1.0 Start fully visible
integer target_visibility = 1; // 1 = visible, 0 = invisible
...
default
{
    state_entry()
    {
        ...
        for (i = 0; i <= prims; i++)
        {
            llSetLinkAlpha(i, 1.0, ALL_SIDES); // Start visible
        }
    ...
    timer()
    {
        ...
        if (now - last_check_time >= CHECK_INTERVAL)
        {
            ...
            target_visibility = !found; // visible only if no one is nearby
        }
